...
const App = () => {
    ...
    return (
      <>
        <div className="w-full flex flex-col h-auto md:h-[88%] space-y-2 relative ">
          ...
          <div
            className="w-14 h-14 bg-blue-700 
                    shadow-sm sticky bottom-12 md:bottom-0 right-0 md:absolute  rounded-full  z-20  flex justify-center  items-center text-white cursor-pointer"
            onClick={geoLocation}
          >
      ...
      <>
        <div className="w-full flex   md:h-[88%] h-[80vh]  justify-center items-center relative flex-col ">
          <div className="flex flex-col space-y-5 justify-center items-center bg-white bg-opacity-90 dark:bg-slate-800 text-center p-4 rounded-2xl max-w-md">
            <h1 className="font-bold text-lg dark:text-gray-100">
              Welcome to <span className="text-blue-700">WeatherSnap !</span>{" "}
              <br />
              Stay updated with the latest weather forecasts and conditions.
            </h1>
            <p className="text-base  dark:text-gray-200 ">
              Stay one step ahead of the weather - find out what's coming your
              way! Use our search option or let us track your current location
              to get accurate and up-to-date weather information.
            </p>
            <button
              className="text-lg p-2 rounded-full bg-blue-700 text-white"
              onClick={geoLocation}
            >
          ...
          <div className="flex justify-end items-end w-full">
            <div
              className="w-16 h-16 bg-blue-700
                    shadow-sm sticky bottom-0 right-0 mt-14 md:mt-0 md:absolute  rounded-full  z-20  flex justify-center  items-center text-white cursor-pointer"
              onClick={geoLocation}
            >
  ...
  function AppBody() {
    return <>{weatherData.length === 0 ? <WelCome /> : <WeatherBody />}</>;
  }
